...
#pragma pack(push,1) // BCAP_VARIANT members need to be aligned correctly
...
typedef enum BCAP_HRESULT {
    ...
    BCAP_S_OK               = 0,  /*	OK                          */
	BCAP_E_TIMEOUT			= 0x80000900,	/*	Timeout				*/
    ...
    BCAP_E_SOCKETWRITEFAIL  = 0x80010006,   /* socket write failed */
    ...
    BCAP_E_UNAVAILABLE      = 0x800401e3,   /* when response is unavailable (probably timed out?) */
...
typedef struct {
    ...
    union {
        ...
        uint32_t LongArray[16];

        void    *Data;                      /* When the Type is VT_Array, Value is stored in *Data */
        //BCAP_VARIANT VariantArray[0]; /// array of variants
...
/* b-CAP Extension Functions */
BCAP_HRESULT    bCap_ExtensionRelease(int iSockFd, uint32_t lhExtension);
BCAP_HRESULT    bCap_ExtensionExecute(int iSockFd, uint32_t lhExtension, const char *pStrCommand, const char *pStrOption, void *plResult);
