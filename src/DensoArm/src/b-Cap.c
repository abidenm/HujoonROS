...
enum {
    ...
    BCAP_INIT_RETRY = 2,
    BCAP_UDP_TIMEOUT =2,
    BCAP_UDP_RETRY = 7,   // CKim - in Sync
    //BCAP_UDP_RETRY = 0,     // CKim -in Async
...
BCAP_HRESULT bCap_RobotExecuteSlaveMove(int iSockFd, uint32_t lhRobot, char *pStrCommand, float *pfOption, void *pResult){
    ...
    if (pSndPacket != NULL) {
        ...
        {   // CKim - Send 8 float (VT_R4) as parameter
            pArg = Arg_Create( VT_R4 | VT_ARRAY, 8, sizeof(float) * 8, pfOption);  // 8 float
            if (pArg != NULL) {
                Packet_AddArg(pSndPacket, pArg);
            }
        }
